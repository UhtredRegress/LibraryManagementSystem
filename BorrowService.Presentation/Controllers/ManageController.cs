...
public class ManageController : ControllerBase
{
    ...
    public async Task<IActionResult> ConfirmBookReturned([FromBody]int borrowHistoryId)
    {
        int userId = Convert.ToInt32(User.FindFirstValue(ClaimTypes.NameIdentifier));
        try
        {
            var result = await _mediator.Send(new ConfirmBookReturnedCommand(userId, borrowHistoryId));
